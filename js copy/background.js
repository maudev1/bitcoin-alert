...
chrome.extension.onConnect.addListener(function (port) {
    // console.log(port);
    port.onMessage.addListener(function (coin) {

        

        if(coin == 1){

            chrome.storage.local.get(['coin'], function (result) {
                console.log('Value currently is ' + result.coin);
        
                port.postMessage(result.coin);
        
            })

        }else{

            chrome.storage.local.set({ coin: coins }, function () {
                console.log('Value is set to ' + coins);
            });
